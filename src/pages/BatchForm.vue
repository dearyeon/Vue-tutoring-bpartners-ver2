				...
				<div class="ibox-content">
									...
									<div class="switch">
										<div class="onoffswitch">
											<input class="onoffswitch-checkbox form-control" name="cancel_bach"
												id="cancel_bach" type="checkbox" v-model="isCancel"/>
						...
						<div class="row">
							<div class="col-md-offset-1 col-lg-4">
								<table class="table">
									<tr>
										<th>BTB 사이트 선택</th>
										<td><input type="text" class="form-control" :placeholder="company" disabled/>
										</td>
									</tr>
									<tr>
										<th>수료기준 출석률</th>
										<td><input type="number" class="form-control" placeholder="출석률을 입력해 주세요."
											v-model="targetRt"/></td>
									</tr>
									<tr>
										<th>자기 부담요율</th>
										<td>
											<input type="text" class="form-control" placeholder="자기 부담금을 입력해 주세요."
												v-model="selfChargeRt"/>
										</td>
									</tr>
								</table>
							</div>
						...
						<div class="row">
							<div class="col-xs-3">
								...
								<select class="col-xs-12" @change="addSelectedGoods($event)">
									...
									<option v-for="(goods,index) in filterdChargePlanList(chargePlanList)" :key="index"
										:value="goods.idx">{{ goods.title }}
									</option>
								</select>
							</div>
							...
							<div class="col-xs-8">
								<table class="table">
									...
									<tr v-for="(item, index) in goodsList" :key="index">
										...
										<td>
											<div class="input-w-text row">
													<span class="col-md-8 no-padding">
														<input class="form-control" type="text" v-model="item.dc_rt"
															oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/>
													</span>
										...
										<td class="text-center">
											<button v-if="!item.idx" class="btn btn-danger"
												@click="deleteSelectedGoods(item.charge_plan.idx)">취소
											</button>
											<button v-if="item.idx" class="btn btn-default" disabled
												@click="deleteSelectedGoods(item.charge_plan.idx)">취소불가
											</button>
										</td>
									</tr>
									</tbody>
					...
					<div class="form-group row">
						...
						<div class="col-sm-10">
							<div class="switch">
								<div class="onoffswitch">
									<input class="onoffswitch-checkbox form-control" name="use_billing" id="use_billing"
										type="checkbox" v-model="useBilling"/>
							...
							<div class="col-xs-6 no-padding">
								<span class="col-xs-5">
									<div class="p-h-xs">
										...
										<date-picker id="chargeDt" name="chargeDt" v-model="chargeDt" type="datetime"
											value-type="YYYY-MM-DD hh:00:00" format="YYYY-MM-DD hh:00:00"
											placeholder="Select date"></date-picker>
									</div>
								</span>
								<span class="col-xs-5">
									<div class="p-h-xs">
										...
										<date-picker id="pChargeDt" name="pChargeDt" v-model="pchargeDt" type="datetime"
											value-type="YYYY-MM-DD hh:00:00" format="YYYY-MM-DD hh:00:00"
											placeholder="Select date"></date-picker>
									</div>
								</span>
					...
					<div class="col-xs-6 pull-right p-h-xl">
						<button class="col-xs-8 col-xs-offset-1 btn btn-lg btn-primary" @click="setBatchInfo(isNew)">
							저장
						</button>
					</div>
			...
			if (parseInt(this.selfChargeRt) > 100) {
				...
				if (this.goodsList.length !== 0 && batchApiRes.result === 2000) {
					...
					for (const item of this.goodsList) {
						if (parseInt(item.dc_rt) > 100) {
							this.$swal('할인율의 최대 수치는 100%입니다.');
							return;
						}
						if (item.idx) {
